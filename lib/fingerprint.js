...
// 真实 Windows 版本号池（sec-ch-ua-platform-version 使用）
const WINDOWS_PLATFORM_VERSIONS = [
  // Windows 10 常见版本
  '10.0.19041', '10.0.19042', '10.0.19043', '10.0.19044', '10.0.19045',
  // Windows 11 常见版本
  '15.0.0', '14.0.0', '13.0.0',
];
...
const COMMON_FONTS = [
  ...
  'Garamond', 'Segoe Print', 'Segoe Script', 'Sylfaen', 'Constantia',
  'Corbel', 'Ebrima', 'Gabriola', 'Leelawadee UI', 'Nirmala UI',
  'Sitka Text', 'Yu Gothic', 'Malgun Gothic', 'Microsoft YaHei',
  'Palatino', 'Rockwell', 'Bahnschrift', 'Ink Free', 'Javanese Text',
];
...
const GEO_FINGERPRINT_MAP = {
  ...
  'SG': { timezones: ['Asia/Singapore'], langs: [['en-SG','en']] },
...
export function generateFingerprintConfig(uuid, geoInfo) {
  ...
  // 按档位关联选择硬件配置，避免不合理组合
  const hwTier = pickOne(['high', 'mid', 'low'], uuid, 'hwTier');
  const tierConfig = {
    high: {
      cores: [8, 12, 16],
      mem: [8, 16],
  ...
  const tier = tierConfig[hwTier];
  const screenCfg = pickOne(tier.screens, uuid, 'screen');
  const hardwareConcurrency = pickOne(tier.cores, uuid, 'cores');
  const deviceMemory = pickOne(tier.mem, uuid, 'mem');
  const devicePixelRatio = pickOne(tier.dpr, uuid, 'dpr');
  ...
  // Windows 平台版本号（用于 sec-ch-ua-platform-version 和 getHighEntropyValues）
  const platformVersion = pickOne(WINDOWS_PLATFORM_VERSIONS, uuid, 'platVer');

  // Canvas 噪声 -5~+5
  const noiseR = Math.floor(hashToNumber(uuid, 'cR') * 11) - 5;
  const noiseG = Math.floor(hashToNumber(uuid, 'cG') * 11) - 5;
  const noiseB = Math.floor(hashToNumber(uuid, 'cB') * 11) - 5;
  ...
  // 字体子集 22-36 种
  const fontCount = 22 + Math.floor(hashToNumber(uuid, 'fontN') * 15);
  ...
  // Not-A.Brand 版本号（真实 Chrome 会变化：8, 24, 99 等）
  const notABrandVer = pickOne(['8', '24', '99'], uuid, 'nab');
  ...
  return {
    ...
    webgl: {
      ...
      shaderPrecisionFormat: {
        FLOAT: { rangeMin: 127, rangeMax: 127, precision: gpuTier === 0 ? 23 : pickOne([23, 15], uuid, 'shaderP') },
        INT:   { rangeMin: 31,  rangeMax: pickOne([30, 31], uuid, 'shaderI'),  precision: 0 },
      },
    },
