...
function _randMinor(min, max) {
  return min + Math.floor(Math.random() * (max - min + 1));
}

function initSessionUA() {
  const sdkMajor = 1;
  const sdkMinor = _randMinor(1, 5);
  const sdkPatch = _randMinor(0, 15);
  const rustMinor = _randMinor(78, 87);
  const rustPatch = 0;
  const apiMinor = _randMinor(50, 95);
  const apiPatch = 0;
  const streamMinor = _randMinor(0, 1);
  const streamPatch = _randMinor(10000, 12000);
  const appVer = `1.${_randMinor(15, 22)}.${_randMinor(0, 9)}`;

  _sessionUA = {
    sdk: `${sdkMajor}.${sdkMinor}.${sdkPatch}`,
    rust: `${rustMinor}.${rustPatch}`,
    api: `${apiMinor}.${apiPatch}`,
    stream: `${streamMinor}.${streamPatch}`,
    appVer,
  };
}

function resetSessionUA() {
  _sessionUA = null;
}

function getSessionUA() {
  if (!_sessionUA) initSessionUA();
  return _sessionUA;
}

/**
 * 获取 OIDC 请求头（模拟 AWS SDK，每会话随机版本号）
 * @returns {Object} 请求头对象
 */
function getOidcHeaders() {
  const ua = getSessionUA();
  const attemptMax = 2 + Math.floor(Math.random() * 3);
  return {
    'Content-Type': 'application/json',
    'User-Agent': `aws-sdk-rust/${ua.sdk} os/windows lang/rust/${ua.rust}`,
    'x-amz-user-agent': `aws-sdk-rust/${ua.sdk} ua/${2 + Math.floor(Math.random() * 2)}.${Math.floor(Math.random() * 3)} api/ssooidc/${ua.api} os/windows lang/rust/${ua.rust} m/E app/AmazonQ-For-CLI`,
    'amz-sdk-request': `attempt=1; max=${attemptMax}`,
    'amz-sdk-invocation-id': generateUUID(),
    // 真实 AWS SDK 会发送 Accept 头
...
class AWSDeviceAuth {
  ...
  async quickAuth() {
    ...
    // 模拟真实 CLI 启动延迟（registerClient 和 deviceAuthorize 之间）
  ...
  async pollToken(timeout = 600000, interval = null) {
    ...
    while (Date.now() - startTime < timeout) {
      ...
      await new Promise(resolve => setTimeout(resolve, pollInterval + Math.random() * pollInterval * 0.3));
    }
  ...
  try {
    ...
    const response = await fetch(`${Q_BASE_URL}/getUsageLimits?origin=AI_EDITOR&resourceType=AGENTIC_REQUEST`, {
      ...
      headers: {
        ...
        'user-agent': `aws-sdk-rust/${ua.sdk} ua/2.1 api/codewhispererstreaming/${ua.stream} os/windows lang/rust/${ua.rust} md/appVersion-${ua.appVer} app/AmazonQ-For-CLI`,
        'x-amz-user-agent': `aws-sdk-rust/${ua.sdk} ua/2.1 api/codewhispererstreaming/${ua.stream} os/windows lang/rust/${ua.rust} m/F app/AmazonQ-For-CLI`,
        'x-amzn-codewhisperer-optout': 'false',
        'authorization': `Bearer ${accessToken}`,
        'amz-sdk-request': `attempt=1; max=${2 + Math.floor(Math.random() * 3)}`,
        'amz-sdk-invocation-id': generateUUID()
      }
    });
...
export { AWSDeviceAuth, OIDC_BASE_URL, getOidcHeaders, validateToken, refreshAndValidateToken, resetSessionUA };
