...
export class ProxyManager {
  ...
  async fetchFromApi(apiUrl) {
    ...
    console.log(`[ProxyManager] 从 API 提取到 ${proxies.length} 个代理`);
    return proxies;
  }
  ...
  getNextProxy() {
    ...
    const proxy = this.proxyPool[this.poolIndex % this.proxyPool.length];
    this.poolIndex++;
    return proxy;
  }
  ...
  async applyProxy(proxyInfo) {
    ...
    if (scheme === 'socks4') {
      console.warn('[ProxyManager] socks4 不支持远程 DNS 解析，存在 DNS 泄露风险，建议使用 socks5');
    }
    ...
    console.log(`[ProxyManager] 代理已设置: ${scheme}://${host}:${port}`);
  }
  /**
   * 设置代理认证处理器
  ...
  async clearProxy() {
    ...
    try {
      ...
      console.log(`[ProxyManager] 代理已清除`);
    } catch (e) {
      console.warn(`[ProxyManager] 清除代理失败:`, e.message);
    }
  }
  ...
  async getGeoLocation(proxyHost) {
    ...
    try {
      ...
      console.log(`[ProxyManager] IP 地理位置: ${proxyHost} -> ${geoInfo.countryCode} (${geoInfo.timezone})`);

      return geoInfo;
    } catch (e) {
      console.warn(`[ProxyManager] IP 地理位置查询失败:`, e.message);
      return null;
    }
  }
}
