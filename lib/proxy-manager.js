...
export class ProxyManager {
   ...
   * @param {string} [forceScheme] 强制使用的协议（如 'socks5'），覆盖解析结果
   */
  parseProxy(proxyStr, forceScheme) {
    ...
    try {
      if (proxyStr.includes('://')) {
        ...
        const scheme = forceScheme || url.protocol.replace(':', '');
        const defaultPort = (scheme === 'socks5' || scheme === 'socks4') ? 1080 : 80;
        return {
          scheme,
          host: url.hostname,
    ...
    if (parts.length >= 2) {
      ...
      return {
        scheme,
        host: parts[0],
        port: parseInt(parts[1]) || (scheme === 'socks5' ? 1080 : 80),
        username: parts[2] || '',
        password: parts[3] || '',
      };
    }
  ...
  /**
   * 设置 Chrome 代理
   * 使用 PAC 脚本实现域名级路由：只有 AWS 相关域名走代理，其余直连
   * 这样 MoeMail API、ipwho.is 等非 AWS 请求不会被代理影响
   */
  async applyProxy(proxyInfo) {
    ...
    // PAC 脚本中的代理类型关键字
    // Chrome 支持: PROXY(HTTP), HTTPS, SOCKS(v4), SOCKS5(v5, 远程DNS)
    const proxyType = scheme === 'socks5' ? 'SOCKS5'
                    : scheme === 'socks4' ? 'SOCKS'
                    : scheme === 'https'  ? 'HTTPS'
                    : 'PROXY';

    // 只代理 AWS 相关域名，其余直连
    // 这样 MoeMail API、ipwho.is 等请求不受代理影响
    const pacScript = `function FindProxyForURL(url, host) {
      if (shExpMatch(host, "*.amazonaws.com") ||
          shExpMatch(host, "*.aws.amazon.com") ||
          shExpMatch(host, "*.signin.aws") ||
          shExpMatch(host, "signin.aws") ||
          shExpMatch(host, "*.awsapps.com") ||
          shExpMatch(host, "*.amazon.com") ||
          shExpMatch(host, "oidc.us-east-1.amazonaws.com")) {
    ...
    // 设置代理认证（HTTP 代理通过 webRequest.onAuthRequired）
    // 注意：SOCKS5 认证在协议层，Chrome 对 SOCKS5 auth 的支持有限
    // 如果 SOCKS5 需要认证，建议使用订阅模式（通过 xray 本地转发）
    if (username && password) {
      this._setupAuthHandler(username, password);
    }

    _log(`[ProxyManager] 代理已设置: ${scheme}://${host}:${port} (仅 AWS 域名)`);
  }
  /**
   * 设置代理认证处理器
