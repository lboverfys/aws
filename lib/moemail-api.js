...
export class MoeMailClient {
  ...
  async createInbox(options = {}) {
    const body = {
      expiryTime: 0,  // 永久（MoeMail 可选值: 3600000/86400000/259200000/0）
    };
    if (this.domain) {
    ...
    _log(`[MoeMail] 创建邮箱: ${this.address} (id: ${this.inboxId})`);
    return this.address;
  }
  ...
  async waitForVerificationCode(timeout = 120000) {
    ...
    // 模拟"去看邮箱"的初始等待
    const initialWait = 3000 + Math.random() * 3000;
    ...
    _log(`[MoeMail] 开始轮询验证码 (超时: ${timeout / 1000}s)`);

    while (Date.now() - startTime < timeout) {
      try {
        const messages = await this._request('GET', `/api/emails/${this.inboxId}?type=received`);
        const msgList = Array.isArray(messages) ? messages : (messages.messages || messages.data || messages.emails || []);
          ...
          for (let i = msgList.length - 1; i >= 0; i--) {
            ...
            if (msg.id && !msg.content && !msg.html && !msg.text) {
              try {
                ...
                _warn(`[MoeMail] 获取邮件详情失败:`, e.message);
              }
            }
          }
        }
      } catch (e) {
        _warn(`[MoeMail] 轮询出错:`, e.message);
      }

      await new Promise(resolve => setTimeout(resolve, pollInterval));
    }

    _warn(`[MoeMail] 验证码获取超时`);
    return null;
  }
  ...
  async deleteInbox() {
    ...
    try {
      ...
      _log(`[MoeMail] 已删除邮箱: ${this.address}`);
    } catch (e) {
      _warn(`[MoeMail] 删除邮箱失败:`, e.message);
    } finally {
      this.inboxId = null;
      this.address = null;
    }
